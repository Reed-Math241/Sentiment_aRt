...
#plot_gg(plot, width = 4, height = 4, scale = 300, multicore = TRUE)
```
...
```{r pinwheel, eval = FALSE}
# https://chichacha.netlify.app/2019/01/29/playing-around-with-phyllotactic-spirals/
...
senti_swirl <- sum(text_afinn$n*text_afinn$value)/sum(text_afinn$n)
...
for(i in 2:500){
  pushViewport(viewport(width = 0.99, height = 0.99, angle = senti_swirl))
...
params <- tibble(polar=TRUE)
params %>% pwalk(.,~generative_art() %>% print())
dev.off()

# imgs_1 <- image_graph(width=600, height=600)
# params <- tibble(polar=TRUE)
# params %>% pmap(.,generative_art)
# dev.off()
