...
# colors$Name <- paste0("^", colors$Name)
# colors$Name <- paste0(colors$Name, "$")
...
book_coords <- text_afinn_unsort %>%
  select(value) %>%
    mutate(sign = sign(value),
         x_coord = cumsum(sign),
         y_coord = cumsum(value))

book_coords_7deg <- text_afinn_unsort %>%
...
feather_sample1 <- sample_n(text_afinn, 10000, replace = TRUE, weight = prob) %>%
  select(value) %>%
  mutate(sign = sign(value),
         x_coord = cumsum(sign),
         y_coord = cumsum(value))

feather_sample2 <- sample_n(text_afinn, 10000, replace = TRUE, weight = prob) %>%
  select(value) %>%
  mutate(sign = sign(value),
         x_coord = cumsum(sign),
         y_coord = cumsum(value))

feather_sample3 <- sample_n(text_afinn, 10000, replace = TRUE, weight = prob) %>%
  select(value) %>%
  mutate(sign = sign(value),
         x_coord = cumsum(sign),
         y_coord = cumsum(value))

feather_sample4 <- sample_n(text_afinn, 10000, replace = TRUE, weight = prob) %>%
  select(value) %>%
  mutate(sign = sign(value),
         x_coord = cumsum(sign),
         y_coord = cumsum(value))

feather_sample5 <- sample_n(text_afinn, 10000, replace = TRUE, weight = prob) %>%
  select(value) %>%
  mutate(sign = sign(value),
         x_coord = cumsum(sign),
         y_coord = cumsum(value))

ggplot(data = feather_sample1, mapping = aes(x = x_coord, y = y_coord)) +
  geom_line() +
...
ggplot(feathers, aes(x = x_coord, y = y_coord)) +
  geom_line() +
  xlim(-100, 10) +
  ylim(-100, 10)
